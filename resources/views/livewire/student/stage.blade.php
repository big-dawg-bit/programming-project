<div class="mx-auto flex max-w-5xl flex-col gap-6">
    ...
    <div class="flex items-center gap-4 rounded-xl border border-red-200 bg-red-50 px-5 py-4">
        <span class="shrink-0 rounded-full bg-blue-50 px-3 py-1 text-xs font-medium text-blue-600">In beoordeling</span>
        <div>
            <p class="font-semibold">{{ $stage->company?->name ?? 'Easi' }}</p>
            <p class="text-sm text-neutral-500">Ingediend op 15 januari 2026</p>
        </div>
    </div>
                ...
                @foreach ($stappen as $stap)
                    <li class="flex gap-4">
                        <div class="flex flex-col items-center">
                            ...
                            @else
                                <span class="size-6 rounded-full border-2 border-neutral-300"></span>
                            @endif
                            @if (! $loop->last)
                                <span class="my-1 w-0.5 flex-1 {{ $stap['state'] === 'done' ? 'bg-green-500' : 'bg-neutral-200' }}"></span>
                            @endif
                        </div>
                        <span class="pb-8 pt-0.5 font-medium {{ $stap['state'] === 'todo' ? 'text-neutral-400' : 'text-neutral-900' }}">
                            {{ $stap['label'] }}
                        </span>
                    </li>
                @endforeach
            ...
            <ul class="space-y-3">
                @foreach ($checklist as $item)
                    <li class="flex items-start gap-2.5 text-sm">
                        ...
                        @else
                            <span class="mt-0.5 size-4 shrink-0 rounded border border-neutral-300"></span>
            ...
            @foreach ($documenten as $doc)
                ...
                <div class="flex items-center gap-3 rounded-lg bg-neutral-50 px-4 py-3">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <p class="text-xs text-neutral-400">{{ $doc['grootte'] }}</p>
                    </div>
                    <span class="rounded-full px-2.5 py-1 text-xs font-medium {{ $ok ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700' }}">
                        {{ $doc['status'] }}
                    </span>
                    <flux:icon.arrow-down-tray class="size-5 shrink-0 text-neutral-400" />
                </div>
            @endforeach
        </div>
    </div>
